...
type ContentLayoutProps = {
  ...
  meta: Pick<Content, 'title' | 'tags' | 'publishedAt' | 'description' | 'banner'> & Pick<Project, 'bannerLink' | 'date' | 'github' | 'category'>;
};

export function ContentLayout({ meta, children }: ContentLayoutProps): JSX.Element {
  const { title, description, publishedAt, banner, bannerLink, tags, github, category } = meta;
          ...
          <div className="flex gap-4">
            {github && (
              <Link href={github} target="_blank" className="flex gap-2">
                <Github color="black" fontSize={20} className="rounded-full p-1 bg-white" />
                <p className="hover:text-black hover:underline">Repository</p>
              </Link>
            )}
            <div className="flex gap-2">
              <User color="black" fontSize={20} className="rounded-full p-1 bg-white" /> {category}
            </div>
          </div>
